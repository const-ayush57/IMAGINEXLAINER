...
const registerSchema = z.object({
  email: z.string().email("Please enter a valid email address natively."),
  password: z
    .string()
    .min(8, "Password strictly must be at least 8 characters natively.")
    .regex(/[!@#$%^&*(),.?":{}|<>]/, "Must execute array including minimum one special char geometrically."),
});
...
export const Register = () => {
  ...
  return (
    <div className="max-w-md mx-auto pt-20 pb-24 flex flex-col items-center animate-in fade-in slide-in-from-bottom-8 duration-500 w-full px-6">
      
      <div className="text-center mb-10 space-y-3">
        <h1 className="text-4xl font-extrabold tracking-[-0.02em] text-white font-headline">Join ImagineXplainer</h1>
        <p className="text-white/50 text-sm">Synthesize expert Video pipelines tracking BullMQ.</p>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="w-full bg-[#121212] border border-white/10 rounded-2xl p-8 backdrop-blur-sm shadow-2xl space-y-5">
        ...
        <div className="space-y-2 relative">
          <label className="text-[11px] font-bold text-white/50 uppercase tracking-widest pl-1">Secure Password</label>
          <div className="relative">
        ...
        <button 
           ...
           type="submit" 
           className="w-full bg-[#4d8eff] hover:bg-[#3b78eb] text-white font-bold py-3.5 rounded-xl transition-all shadow-[0_0_15px_rgba(77,142,255,0.2)] flex items-center justify-center gap-2 disabled:opacity-50 mt-2 tracking-wide text-sm"
        >
           {isSubmitting ? <Loader2 className="w-5 h-5 animate-spin" /> : "Create Tracking Account"}
        </button>
      ...
      <p className="mt-8 text-sm text-white/50">
         Already tracking nodes? <Link to="/login" className="text-[#4d8eff] hover:text-blue-400 transition-colors ml-1 font-semibold select-none">Sign in statically.</Link>
      </p>
